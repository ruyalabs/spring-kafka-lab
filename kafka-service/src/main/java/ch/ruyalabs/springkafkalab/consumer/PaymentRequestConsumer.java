...
public class PaymentRequestConsumer {
    ...
    public void processPayment(
        ...
        // Generate transaction ID and send success response
        String transactionId = UUID.randomUUID().toString();
        String successMessage = String.format("Payment %s processed successfully. Amount: %s %s", 
                                             payment.getId(), payment.getAmount(), payment.getCurrency());

        paymentResponseProducer.sendSuccessResponse(payment.getId(), transactionId, successMessage);
        log.info("Sent success response for payment: {} with transaction ID: {}", payment.getId(), transactionId);
